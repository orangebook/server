...
bool mysql_create_or_drop_trigger(THD *thd, TABLE_LIST *tables, bool create)
{
  ...
  */
  ...
  if (!result)
  {
    result= write_bin_log(thd, TRUE, stmt_query.c_ptr(), stmt_query.length());
  }
                                         ...
                                         String *stmt_query)
{
  ...
  */
  ...
  if (!access(trigname_buff, F_OK))
  {
    if (lex->create_info.or_replace())
    {
      ...
      /*
        The following can fail if there exists a .TRN file but there
        was no trigger for it in the .TRG file
      */
    ...
    else if (lex->create_info.if_not_exists())
    {
      ...
      String trigger_def;
      /* Update query for binary logging */
  ...
  */
  ...
  trigger->create_time= thd->start_time*100 + thd->start_time_sec_part/1000;
                                      ...
                                      Trigger *trigger)
{
  ...
  if (ordering_clause != TRG_ORDER_NONE && !found_trigger)
  {
    my_error(ER_REFERENCED_TRG_DOES_NOT_EXIST, MYF(0),
             anchor_trigger_name->str);
    return true;
  }
                                           ...
                                           bool old_row_is_record1)
{
  ...
  Trigger *trigger= get_trigger(event, time_type);
